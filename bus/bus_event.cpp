...
/*
    ...
    int32_t bus_event_t::parse_event(char* buf, uint32_t& pos, uint32_t packlen, bus_user_process* pUserProcess) {
        ...
        int ret = -1;
        // 更新binlog的pos
        if (event_head.log_pos != 0) {// 若event类型为rotate或者binlog中不存在fileformate event,则包体中logpos =0,此时logpos-event_sz会造成负数
            uint32_t next_binlog_offset = event_head.log_pos;// log_pos下一个事件的位置
            std::string &next_binlog_name = rotate_event.get_binlog_filename();// 下一个binlog filename
            ...
            g_logger.debug("type: %d, logfilename: %s, curpos: %lu, nextlogpos: %lu", event_head.event_type,
                           rotate_event.get_binlog_filename().c_str(), event_head.log_pos - packlen + 1,
                           event_head.log_pos);
#if 0
    ...
    if (m_bIsChecksumEnable == true && format_event.get_alg() != BINLOG_CHECKSUM_ALG_OFF && format_event.get_alg() != BINLOG_CHECKSUM_ALG_UNDEF)
        this->pack_len -= BINLOG_CHECKSUM_LEN;
...
#if 1
                // 清理部分内存，防止申请太对内存造成内存溢出
                bus_table_map_t* pMapEvent = nullptr;
            ...
            case 0x13:
                ...
                if (m_bIsChecksumEnable == true && format_event.get_alg() != BINLOG_CHECKSUM_ALG_OFF && format_event.get_alg() != BINLOG_CHECKSUM_ALG_UNDEF)
                    ...
                    if (it == map_map_event.end()) {
                        ...
                        if (pMapEvent == nullptr) {
                            g_logger.error("pMapEvent == nullptr, return");
                            return 0;
                        }
                    ...
                    if (pMapEvent->parse_map_event_body(buf, pos, *this) != 0) {
            ...
            case 0x17:
                {
                    printf("--- 事件类型：write rows event v1!\n");
                ...
                if (m_bIsChecksumEnable == true && format_event.get_alg() != BINLOG_CHECKSUM_ALG_OFF && format_event.get_alg() != BINLOG_CHECKSUM_ALG_UNDEF)
                    ...
                    if (ret == -1) {
                        g_logger.error("parse write event v1 fail");
                        return -1;
                    } else if (ret == 1) {
            ...
            case 0x19:
                {
                    printf("--- 事件类型：delete rows event v1!\n");
            ...
            case 0x1E:
                ...
                if (m_bIsChecksumEnable == true && format_event.get_alg() != BINLOG_CHECKSUM_ALG_OFF && format_event.get_alg() != BINLOG_CHECKSUM_ALG_UNDEF)
                    ...
                    ret = row_event.parse_row_event_body(buf, pos, *this, pUserProcess);
                    if (ret == -1) {
                        g_logger.error("parse write event v2 fail");
                        return -1;
                    } else if (ret == 1) {
                        g_logger.notice("write row event v2, recv stop signal");
                        return 1;
                    }
                }
                break;
            case 0x1F:
                ...
                if (m_bIsChecksumEnable == true && format_event.get_alg() != BINLOG_CHECKSUM_ALG_OFF && format_event.get_alg() != BINLOG_CHECKSUM_ALG_UNDEF)
                    ...
                    if (ret == -1) {
                        ...
                        g_logger.notice("update row  event v2, recv stop signal");
                        return 1;
                    }
                }
                break;
            case 0x20:
                ...
                if (m_bIsChecksumEnable == true && format_event.get_alg() != BINLOG_CHECKSUM_ALG_OFF && format_event.get_alg() != BINLOG_CHECKSUM_ALG_UNDEF)
                    ...
                    ret = row_event.parse_row_event_body(buf, pos, *this, pUserProcess);
                    if (ret == -1) {
                        g_logger.error("parse delete event v2 fail");
                        return -1;
                    } else if (ret == 1) {
                        g_logger.notice("delete row event v2, recv stop signal");
                        return 1;
                    }
                }
                break;
    ...
    /***********************************************************/
    int32_t bus_row_event_t::parse_row_event_body(char* buf, uint32_t& pos, bus_event_t& cur_event, bus_user_process* pUserProcess) {
        uint8_t post_head_len = cur_event.get_event_posthead_len(_cmd);
        if (post_head_len == 6) {
            _table_id = uint4korr(buf + pos);
            pos += 4;
        ...
        if (cur_event.map_map_event.find(_table_id) == cur_event.map_map_event.end()) {
            g_logger.error("no table map event for this row event");
            return -1;
        }
        ...
        if (_table_id != pMapEvent->get_tableid()) {
            g_logger.error("_table_id: %lu, pMapEvent->get_table_id(): %lu", _table_id, pMapEvent->get_tableid());
            return 0;// why 0?
        }
        ...
        while (cur_event.pack_len > pos) {
            ...
            if (row == nullptr) oom_error();

            // 配置database name和table name
            row->set_db(pMapEvent->get_database_name().c_str());
            row->set_table(pMapEvent->get_table_name().c_str());

            if (_cmd == 0x17 || _cmd == 0x1E) {// write rows event
                row->set_action(INSERT);
                // 分析行的数据信息
                if (!parse_row(buf, pos, null_bitmap1_size, row, _present_bitmap1, false, cur_event, *pMapEvent)) {
            ...
            } else if (_cmd == 0x19 || _cmd == 0x20) {// delete rows event
                row->set_action(DEL);
            ...
            } else if (_cmd == 0x18 || _cmd == 0x1F) {// update rows event
        ...
        return 0;
    }// bus_row_event_t::parse_row_event_body()
