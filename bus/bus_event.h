...
typedef unsigned char uchar;// 可以保存两倍数量的有符号数
...
#define MYSQL_TYPE_TIME2 19
    ...
    public:
        ...
        uint32_t get_event_pos() {// 获取event pos
            return log_pos - event_sz;// 当前的event记录在binlog文件中的位置--具体而言就是哪一行
        }
    ...
    public:
        bus_rotate_event_t() {}
        ~bus_rotate_event_t() {}
        int32_t parse_rotate_event_body(char* buf, uint32_t& pos, bus_event_t& cur_event);
    ...
    private:
        ...
        uint64_t _next_pos;// 下一个event事件在下一个binlog文件中的位置
        std::string _next_binlog_filename;// 下一个binlog文件名
    };
    ...
    public:
        ...
        ~bus_format_event_t() {}
        int32_t parse_format_event_body(char* buf, uint32_t& pos, bus_event_t& cur_event, bool isChecksumEnable);
...
//        bus_table_map_t() {_cmd = 0x13;}
        ~bus_table_map_t() {}
        int32_t parse_map_event_body(char* buf, uint32_t& pos, bus_event_t& cur_event);
        void print_info() {
    ...
    private:
        ...
        bus_mem_block_t _null_bitmap;// ??位字段,指示每个列是否可以为空,每个列一位
    };
    ...
    public:
        ...
        ~bus_row_event_t() {}
        int32_t parse_row_event_body(char* buf, uint32_t& pos, bus_event_t& cur_event, bus_user_process* pUserProcess);
...
//            if (map_schema != nullptr) {
//                delete map_schema;
//                map_schema = nullptr;
//            }
        }

        int32_t parse_event(char* buf, uint32_t& pos, uint32_t packlen, bus_user_process* pUserProcess);
    ...
    public:
        ...
        std::map<uint64_t, bus_table_map_t*> map_map_event;// table_map_event
