...
namespace bus {
    ...
    {
        printf("开始bus_packet_t::read_packet()!\n");
        ...
        while (cur_payload_sz == 0xffffff)
        {
            ...
            char buf[4];// 读到buf中
            bytes_ct = 0;
            printf("读取packet headwhile()之前!\n");
//            uint32_t uNowTime = 0;
            while (bytes_ct != 4) {
//                uint32_t uLastFullPullTime = time(0);
                printf("进入packet headwhile(),read()函数之前!\n");
//                printf("bytes_ct: %d\n",bytes_ct);
//                printf("buf + bytes_ct: %x\n",buf + bytes_ct);
//                uNowTime = time(0);
//                if (uNowTime - uLastFullPullTime > 5) {
//                    printf("packet head读取超时,break\n");
//                    break;
//                }
                // 会陷入阻塞
                ret = read(fd, buf + bytes_ct, 4 - bytes_ct);
                printf("进入packet headwhile(),read()函数之后!\n");
                if (ret == 0) {
                    printf("读取packet head-read socket eof\n");
                    g_logger.notice("read socket eof");
                    return -1;
                } else if (ret == -1) {
                    printf("读取packet head-read packet head fail, error:%s\n", strerror(errno));
                    g_logger.error("read packet head fail, error:%s", strerror(errno));
                    return -1;
                } else {
                    printf("读取packet head-读取到的packet head的大小: %d\n",ret);
                    bytes_ct += ret;
                }
            }
            buf[4] = '\0';
            printf("读取packet head后的buf: %s\n",buf);
            std::cout << "读取packet head后的buf: " << buf << std::endl;

//            cur_payload_sz = uint3korr(buf);// 这是啥？？？--为了计算buf的大小
            cur_payload_sz = uint3korr(buf);

            printf("一开始的cur_payload_sz: %d\n",cur_payload_sz);
            /* 分配空间 */
            _add_mem(cur_payload_sz);
            /*读取body */
            bytes_ct = 0;
            int i = 1;
            printf("读取body while()之前!\n");
            while(bytes_ct != cur_payload_sz) {
                printf("这是第 %d 次read body\n",i);
                // 问题：_body + _bodylen + bytes_ct来自哪里
                // cur_payload_sz - bytes_ct是多大？
                printf("bytes_ct: %d, cur_payload_sz: %d\n",bytes_ct, cur_payload_sz);
                printf("cur_payload_sz - bytes_ct: %d\n",cur_payload_sz - bytes_ct);
                ret = read(fd, _body + _bodylen + bytes_ct, cur_payload_sz - bytes_ct);
                if (ret == 0) {
                    // 问题出在这里！！！
                    // 读取长度大于文件实际长度
                    printf("读取body-read socket EOF!\n");
                    g_logger.notice("read socket eof");
//                    break;
//                    return 0;
                    return -1;
                } else if (ret == -1) {
                    printf("读取body-read packet body fail, error:%s\n", strerror(errno));
                    g_logger.error("read packet body fail, error:%s", strerror(errno));
                    return -1;
                } else {
                    //g_logger.notice("sz=%d,count=%d", cur_payload_sz, ret);
                    printf("第 %d 次read body的大小: %d\n",i, ret);
                    bytes_ct += ret;
                }
                i++;
            }
        ...
        printf("结束bus_packet_t::read_packet()!\n");
        return 0;
    }
...
//            printf("开始parse_event之前!\n");
            ...
            *pUserProcess)
    {
        ...
        printf("开始event_head.parse_event_head\n");
        ...
        if (event_head.log_pos != 0)
        {
            ...
            printf("***** 更新binlog的position:\ntype: %x(16进制), logfilename:%s, curpos:%lu, nextlogpos:%lu\n", event_head.event_type,
        ...
        switch(event_head.event_type)
        {
            ...
            case 0x0f:
                {
                    printf("------- 事件类型:format description event!\n");
                    ...
                    {
                        printf("delete bus_table_map_t, table_id:%lu\n", it->first);
            ...
            case 0x13:
                {
                    ...
                    printf("------- 事件类型:table map event, [%s:%lu]!\n",binlog_name.c_str(), offset);
                    printf("table map event, [%s:%lu]\n", binlog_name.c_str(), offset);
                    ...
                    if (it == map_map_event.end())
                    {
                        ...
                        printf("new bus_table_map_t, table_id:%lu\n", table_id);
            ...
            case 0x17:
                {
                    ...
                    {
                        printf("row_event.parse_row_event_body--write row event, recv stop signal");
                        g_logger.notice("write row event, recv stop signal");
                        return 1;
                    } else {
                        printf("row_event.parse_row_event_body成功!\n");
                    }
                }
                break;
            case 0x18:
                {
                    printf("update rows event v1, [%s:%lu]\n",
                           map_position.binlog_file, map_position.binlog_offset);
            ...
            case 0x19:
                {
                    printf("------- 事件类型--delete rows event v1, [%s:%lu]\n",
                           map_position.binlog_file, map_position.binlog_offset);
            ...
            case 0x1E:
                {
                    printf("------- 事件类型:write rows event v2, [%s:%lu]!\n",map_position.binlog_file, map_position.binlog_offset);
                    ...
                    } else if (ret == 1)
                    {
                        ...
                        printf("write rows event事件row_event.parse_row_event_body成功!\n");
                    }
                }
                break;
            case 0x1F:
                {
                    printf("------- 事件类型--update rows event v2, [%s:%lu]\n",
                           map_position.binlog_file, map_position.binlog_offset);
            ...
            case 0x20:
                {
                    printf("事件类型--delete rows event v2, [%s:%lu]", map_position.binlog_file, map_position.binlog_offset);
    ...
    {
        printf("******* 进入bus_row_event_t::parse_row_event_body!\n");
        ...
        {
            printf("_table_id:%lu, pMapEvent->get_tableid():%lu\n", _table_id, pMapEvent->get_tableid());
...
//                printf("row->set_action之后\n");
                ...
                if (!parse_row(buf, pos, null_bitmap1_size, row, _present_bitmap1, false, cur_event,
                            *pMapEvent))
                {
                    printf("****** bus_row_event_t::parse_row_event_body内部 DEL 错误!\n");
                    g_logger.error("parse [%s:%lu] row fail",
                            cur_event.map_position.binlog_file,
                            cur_event.map_position.binlog_offset);
                ...
                if (!parse_row(buf, pos, null_bitmap1_size, row, _present_bitmap1, true, cur_event,
                            *pMapEvent))
                {
                    printf("****** bus_row_event_t::parse_row_event_body内部 UPDATE 错误!\n");
                    g_logger.error("parse [%s:%lu] row fail",
                            cur_event.map_position.binlog_file,
                            cur_event.map_position.binlog_offset);
                    if (row)
            ...
            if (pUserProcess) {
                printf("开始pUserProcess->IncrProcess(row)!\n");
                pUserProcess->IncrProcess(row);
            } else {
                printf("不进行IncrProcess!\n");
            }
#if 0
            /*
               typedef struct 
...
//        printf("****** 退出while (cur_event.pack_len > pos)\n");

//        printf("******* 退出bus_row_event_t::parse_row_event_body!\n");
        return 0;
    }
...
//        printf("convert_charset_t& convert = cur_event.map_convert;之后\n");
...
//        printf("schema_t *curschema = cur_event.map_schema;之后\n");
        ...
        for (uint32_t i = 0; i < _column_count; ++i)
        {
            if (present_bitmap.get_bit(i)) {
                ...
                if (null_col) {
                    ...
                    //column_t *col = NULL;
//                    printf("if (!parse_column_value之前\n");
...
//                    } else {
//                        printf("if (!parse_column_value没有问题\n");
//                    }
                }
                ++present_pos;
            } else {
