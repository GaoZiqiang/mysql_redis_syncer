...
namespace bus
{
    ...
    int MyRedis::Init(const char* ip, int port, bool bRedisNeedPassword, const char *szRedisPassword)
    {
        ...
        printf("初始化后的redis参数--ip :%s port :%d\n", m_szIp, m_hdPort);
        // 进行redis连接
    ...
    int MyRedis::RedisCmdNoReply(const char* pszFormat, ...)
    {
        ...
        do
        {
            ...
            // 执行redis命令
//            printf("RedisCmdNoReply-pszFormat为：%s",pszFormat)--hmset informhubsvr_binlog_pos_hash filename %s pos %d
...
//                            printf("插入redis成功!\n");
    ...
    int MyRedis::RedisCmdWithReply(const char* pszFormat, ...)
    {
        ...
        do
        {
            ...
            va_start( argList, pszFormat );
//            printf("RedisCmdWithReply-pszFormat为：%s\n",pszFormat);
