...
//        printf("mysql ip为: %s,端口为: %s,用户名为: %s\n",pConfig->_mysql_ip,pConfig->_mysql_port,pConfig->_mysql_username);
        return nRet;
    }
    ...
    {
        printf("现在在inform_process.c bus_inform_process::IncrProcess\n");
        printf("action:%d, database:%s, table:%s, column_count:%d\n", row->get_action(),
               row->get_db_name(), row->get_table(), row->size());
        ...
        {
            printf("找到d_ec_user-t_user_group!\n");
    ...
    int bus_inform_process::FullPullFriended()
    {
        ...
        while (1)
        {
            ...
            if (nRet != 0)
            {
                printf("mysql_real_query失败Failed to mysql_real_query errstr: %s\n", mysql_error(&m_mysql));
 ...
 */
    ...
    int bus_inform_process::IncrProcessFriendedAndStaff(row_t *row)
    {
        ...
        if (nType == 0)
        {
            ...
            {
                printf("添加数据!\n");
                printf("添加记录到redis--sadd friended_set:%d %d\n",nGroupId, nUserId);
                m_pRedis->RedisCmdNoReply("sadd friended_set:%d %d", nGroupId, nUserId);
            }
            else if (row->get_action() == 1)// 更新数据
            {
                printf("更新数据!\n");
